...
# NGN2 focus
upbed_ngn2_with_epigenetic_marks <- file.path(input_path, "NGN2_with_epigenetic_marks_increasing.bed")
downbed_ngn2_with_epigenetic_marks <- file.path(input_path, "NGN2_with_epigenetic_marks_decreasing.bed")

data_up_ngn2_with_epigenetic_marks <- read.table(upbed_ngn2_with_epigenetic_marks, header = F, sep = "\t", col.names = c("chr", "start", "end", "name", "strand", "score"))
data_down_ngn2_with_epigentic_marks <- read.table(downbed_ngn2_with_epigenetic_marks, header = F, sep = "\t", col.names = c("chr", "start", "end", "name", "strand", "score"))

ngn2_increasing_activity_regions_with_epigenetic_marks <- GenomicRanges::makeGRangesFromDataFrame(data_up_ngn2_with_epigenetic_marks, seqnames.field = "chr", start.field = "start", end.field = "end")
ngn2_decreasing_activity_regions_with_epigenetic_marks <- GenomicRanges::makeGRangesFromDataFrame(data_down_ngn2_with_epigentic_marks, seqnames.field = "chr", start.field = "start", end.field = "end")


# undiffWTC11 focus
upbed_NGN2_without_epigenetic_marks <- file.path(input_path, "NGN2_without_epigenetic_marks_increasing.bed")
downbed_NGN2_without_epigenetic_marks <- file.path(input_path, "NGN2_without_epigenetic_marks_decreasing.bed")

data_up_NGN2_without_epigenetic_marks <- read.table(upbed_NGN2_without_epigenetic_marks, header = F, sep = "\t", col.names = c("chr", "start", "end", "name", "strand", "score"))
data_down_NGN2_without_epigenetic_marks <- read.table(downbed_NGN2_without_epigenetic_marks, header = F, sep = "\t", col.names = c("chr", "start", "end", "name", "strand", "score"))

ngn2_increasing_activity_regions_without_epigenetic_marks <- GenomicRanges::makeGRangesFromDataFrame(data_up_NGN2_without_epigenetic_marks, seqnames.field = "chr", start.field = "start", end.field = "end")
ngn2_decreasing_activity_regions_without_epigenetic_marks <- GenomicRanges::makeGRangesFromDataFrame(data_down_NGN2_without_epigenetic_marks, seqnames.field = "chr", start.field = "start", end.field = "end")
...
# Combine the sequence lists
ngn2_dCREs_with_and_without_epigenetic_marks_and_background <- list(Neuron_with_epigenetic_marks_increasing = ngn2_increasing_activity_regions_with_epigenetic_marks, Neuron_with_epigenetic_marks_decreasing = ngn2_decreasing_activity_regions_with_epigenetic_marks, Neuron_closed_chromatin_increasing = ngn2_increasing_activity_regions_without_epigenetic_marks, Neuron_closed_chromatin_decreasing = ngn2_decreasing_activity_regions_without_epigenetic_marks, background_conservative = both_cell_types_around_zero_activity_001)
...
ngn2_remap_chip_epigenetic_marks_increasing_decreasing <- crosswisePermTest(
    Alist = ngn2_dCREs_with_and_without_epigenetic_marks_and_background,
    Blist = tflist,
    sampling = TRUE,
...
# # WITH background sequences
ngn2_remap_chip_epigenetic_marks_increasing_decreasing_CrosswiseMatrix <- makeCrosswiseMatrix(ngn2_remap_chip_epigenetic_marks_increasing_decreasing, clusterize = TRUE, transform =TRUE) # clusterize = F, transform = T) # , transform=T) #, clusterize=T)

output_name <- "ngn2_dCREs_with_and_without_epigenetic_marks_sampling_1000_with_background_full_remap_TF_chip_increasing_decreasing_CrosswiseMatrix.rds"

# Use file.path() to combine directory and file name
output_path <- file.path(output_directory, output_name)
saveRDS(ngn2_remap_chip_epigenetic_marks_increasing_decreasing_CrosswiseMatrix, file = output_path)

# # Extract the two datasets
# up_data <- ngn2_undiffWTC11_remap_chip_increasing_decreasing_CrosswiseMatrix@multiOverlaps$Neuron_upregulating_dCREs %>%
#     mutate(regulation = "increasing transcription") # Add a column to label the dataset
# down_data <- ngn2_undiffWTC11_remap_chip_increasing_decreasing_CrosswiseMatrix@multiOverlaps$Neuron_downregulating_dCREs %>%
...
# # Plot the data
# pdf_name_enhancer <- "ngn2_undiffWTC11_sampling_1000_dotplot_enhancer_with_background_full_remap2022_TF_enrichment.pdf"
...
# # Plot the data
# pdf_name_silencer <- "ngn2_undiffWTC11_sampling_1000_dotplot_silencer_with_background_full_remap2022_TF_enrichment.pdf"
