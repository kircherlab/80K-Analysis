...
# NGN2 focus
upbed_ngn2 <- file.path(input_path, "ngn2_increasing_regions_duplicates_without_considering_strand.bed")
downbed_ngn2 <- file.path(input_path, "ngn2_decreasing_regions_duplicates_without_considering_strand.bed")

data_up_ngn2 <- read.table(upbed_ngn2, header = F, sep = "\t", col.names = c("chr", "start", "end", "name", "strand", "score"))
data_down_ngn2 <- read.table(downbed_ngn2, header = F, sep = "\t", col.names = c("chr", "start", "end", "name", "strand", "score"))

ngn2_increasing_activity_regions <- GenomicRanges::makeGRangesFromDataFrame(data_up_ngn2, seqnames.field = "chr", start.field = "start", end.field = "end")
ngn2_decreasing_activity_regions <- GenomicRanges::makeGRangesFromDataFrame(data_down_ngn2, seqnames.field = "chr", start.field = "start", end.field = "end")


# undiffWTC11 focus
upbed_undiffWTC11 <- file.path(input_path, "undiffWTC11_increasing_regions_no_duplicates.bed")
downbed_undiffWTC11 <- file.path(input_path, "undiffWTC11_decreasing_regions_no_duplicates.bed")

data_up_undiffWTC11 <- read.table(upbed_undiffWTC11, header = F, sep = "\t", col.names = c("chr", "start", "end", "name", "strand", "score"))
data_down_undiffWTC11 <- read.table(downbed_undiffWTC11, header = F, sep = "\t", col.names = c("chr", "start", "end", "name", "strand", "score"))

undiffWTC11_increasing_activity_regions <- GenomicRanges::makeGRangesFromDataFrame(data_up_undiffWTC11, seqnames.field = "chr", start.field = "start", end.field = "end")
undiffWTC11_decreasing_activity_regions <- GenomicRanges::makeGRangesFromDataFrame(data_down_undiffWTC11, seqnames.field = "chr", start.field = "start", end.field = "end")
...
ngn2_dCREs_undiffWTC11_dCREs_and_background <- list(Neuron_upregulating_dCREs = ngn2_increasing_activity_regions, Neuron_downregulating_dCREs = ngn2_decreasing_activity_regions, undiffWTC11_upregulating_dCREs = undiffWTC11_increasing_activity_regions, undiffWTC11_downregulating_dCREs = undiffWTC11_decreasing_activity_regions, background_conservative = both_cell_types_around_zero_activity_001)
...
ngn2_undiffWTC11_remap_chip_increasing_decreasing <- crosswisePermTest(
    Alist = ngn2_dCREs_undiffWTC11_dCREs_and_background,
    Blist = tflist,
    sampling = TRUE,
...
# # WITH background sequences
ngn2_undiffWTC11_remap_chip_increasing_decreasing_CrosswiseMatrix <- makeCrosswiseMatrix(ngn2_undiffWTC11_remap_chip_increasing_decreasing, clusterize = TRUE, transform =TRUE) # clusterize = F, transform = T) # , transform=T) #, clusterize=T)

output_name <- "ngn2_undiffWTC11_sampling_1000_with_background_full_remap_TF_chip_increasing_decreasing_CrosswiseMatrix.rds"

# Use file.path() to combine directory and file name
output_path <- file.path(output_directory, output_name)
saveRDS(ngn2_undiffWTC11_remap_chip_increasing_decreasing_CrosswiseMatrix, file = output_path)
