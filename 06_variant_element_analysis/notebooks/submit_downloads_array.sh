#!/bin/bash
# submit_downloads_array.sh

# Slurm directives
#SBATCH --job-name=gwas_array_dl   # Job array name
#SBATCH --output=slurm_logs/gwas_array_%A_%a.out  # Standard output file (%A: Array Job ID, %a: Array Task ID)
...
#SBATCH --array=1-20 # 1-4587 # Task IDs will be 1 to NUM_TASKS

# Configuration
MANIFEST_FILE="UKBB_GWAS_Imputed_v3_manifest_release_20180731_filtered.tsv"           # Your input manifest table file
OUTPUT_DIR="filtered_gwas_results_slurm"    # Directory to store results
PVAL_COLUMN_IN_GWAS_FILE=11                 # P-value column index in the downloaded GWAS files
