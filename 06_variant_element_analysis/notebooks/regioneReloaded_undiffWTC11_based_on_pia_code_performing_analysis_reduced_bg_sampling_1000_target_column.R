...
### undiffWTC11 focus: undiffWTC11 dCREs are not filtered out + very small subset as background set
...
# undiffWTC11 focus
upbed <- file.path(input_path, "undiffWTC11_increasing_regions_no_duplicates.bed")
downbed <- file.path(input_path, "undiffWTC11_decreasing_regions_no_duplicates.bed")
...
undiffWTC11_increasing_activity_regions <- GenomicRanges::makeGRangesFromDataFrame(data_up, seqnames.field = "chr", start.field = "start", end.field = "end")
undiffWTC11_decreasing_activity_regions <- GenomicRanges::makeGRangesFromDataFrame(data_down, seqnames.field = "chr", start.field = "start", end.field = "end")
# undiffWTC11_dCREs <- list(undiffWTC11_upregulating_dCREs = undiffWTC11_increasing_activity_regions, undiffWTC11_downregulating_dCREs = undiffWTC11_decreasing_activity_regions)
...
undiffWTC11_dCREs_and_background <- list(undiffWTC11_upregulating_dCREs = undiffWTC11_increasing_activity_regions, undiffWTC11_downregulating_dCREs = undiffWTC11_decreasing_activity_regions, background_conservative = both_cell_types_around_zero_activity_001)
...
undiffWTC11_remap_chip_increasing_decreasing <- crosswisePermTest(
    Alist = undiffWTC11_dCREs_and_background,
    Blist = tflist,
    sampling = TRUE,
...
# # WITH background sequences
undiffWTC11_remap_chip_increasing_decreasing_CrosswiseMatrix <- makeCrosswiseMatrix(undiffWTC11_remap_chip_increasing_decreasing, clusterize = TRUE, transform =TRUE) # clusterize = F, transform = T) # , transform=T) #, clusterize=T)

# # WITHOUT background sequences
# # set clusterize to FALSE if only 2 sequence sets => here more then 2 sequence sets
# undiffWTC11_remap_chip_increasing_decreasing_CrosswiseMatrix <- makeCrosswiseMatrix(undiffWTC11_remap_chip_increasing_decreasing, clusterize = TRUE, transform = TRUE) # clusterize = F, transform = T) # , transform=T) #, clusterize=T)

output_name <- "sampling_1000_undiffWTC11_with_background_full_remap_TF_chip_increasing_decreasing_CrosswiseMatrix.rds"

# Use file.path() to combine directory and file name
output_path <- file.path(output_directory, output_name)
saveRDS(undiffWTC11_remap_chip_increasing_decreasing_CrosswiseMatrix, file = output_path)

# Extract the two datasets
up_data <- undiffWTC11_remap_chip_increasing_decreasing_CrosswiseMatrix@multiOverlaps$undiffWTC11_upregulating_dCREs %>%
    mutate(regulation = "increasing transcription") # Add a column to label the dataset
down_data <- undiffWTC11_remap_chip_increasing_decreasing_CrosswiseMatrix@multiOverlaps$undiffWTC11_downregulating_dCREs %>%
...
# Plot the data
pdf_name_enhancer <- "undiffWTC11_sampling_1000_dotplot_enhancer_with_background_full_remap2022_TF_enrichment.pdf"
...
# Plot the data
pdf_name_silencer <- "undiffWTC11_sampling_1000_dotplot_silencer_with_background_full_remap2022_TF_enrichment.pdf"
