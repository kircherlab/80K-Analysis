...
with open(config["files"]["assignment_tbl"], "w") as output_file, open(config["files"]["output_log"], "w") as read_log:
      ...
      if read.flag == 16:
        ...
        if read.get_tag("AS") == read.get_tag("XS"):
          try: 
            ...
            read_log.write("WARNING: read (%s) can not be rescued because XA tag is not given\n"%(read.query_name))
          ...
          if forward_matches == 1: # best alignment found on forward strand
            ...
            read_log.write("WARNING: read (%s) can not be rescued because more than one alignment on forward strand is given\n"%(read.query_name))
      ...
      if read.flag == 16:
        ...
        if read.get_tag("AS") == read.get_tag("XS"):
          ...
          try:
            ...
            read_log.write("WARNING: read (%s) can not be rescued because XA tag is not given"%(read.query_name))
            count_no_second_alignment += 1
            continue  
...
# read config["files"]["assignment_tbl"] in pandas
df = pandas.read_csv(config["files"]["assignment_tbl"], sep="\t", header=None)
# sort by first column
df = df.sort_values(by=[0])
# write to file
df.to_csv(config["files"]["assignment_tbl"], sep="\t", header=False, index=False)
...
if config["general"]["verbose"]:
  ...
  print("All rescued reads: ", change_quality_count + rescued_read_count) # All rescued reads
  print("Found alignment on forward strand: ", rescued_read_count) # rescued reversed reads
  print("This alignment is a best alignment (score of second best alignment is lower): ", change_quality_count) # rescued becasue it is the best alignment
  print("-- Not rescued alignments --")
  print("Number of unmapped reads: ", unmapped_count) # not rescued 
