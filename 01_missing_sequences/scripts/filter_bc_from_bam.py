...
@click.option('--verbose',
                ...
                default = True,
                type = bool,
                help = 'Specify if you would like to receive warnings in standard output (default=True).')
@click.option('--output',
                '-o',
                ('output_path'),
                required = True,
        ...
        if read.flag == 16:
          ...
          if read.get_tag("AS") == read.get_tag("XS"):
            try: 
              ...
              print("WARNING: read (%s) can not be rescued because XA tag is not given\n"%(read.query_name))
            ...
            if forward_matches == 1: # best alignment found on forward strand
              ...
              print("WARNING: read (%s) can not be rescued because more than one alignment on forward strand is given\n"%(read.query_name))
      
      else: # mapping quality >= 1
        if not sequence_identity >= identity_threshold: high_mapping_quality_low_identity += 1
        ...
        if read.flag == 16:
          ...
          if read.get_tag("AS") == read.get_tag("XS"):
            ...
            try:
              ...
              print("WARNING: read (%s) can not be rescued because XA tag is not given"%(read.query_name))
              count_no_second_alignment += 1
              continue  
    if verbose:
