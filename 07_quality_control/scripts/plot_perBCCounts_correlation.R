...
# save.image(file='qc_barcode_plots_rna_dna.RData')
load('qc_barcode_plots_rna_dna.RData')
...
#   outdir <- opt$outdir
# }
#### test locally so add the parameters you want: 
# only two replicates: 
opt <- list(
  condition = "NGN2",
  files = "C:\\Users\\user\\OneDrive - Charité - Universitätsmedizin Berlin\\Documents\\coding\\80K_analysis\\qc_plots\\barcodes\\NGN2_3.merged.config.standardConfig.tsv.gz,C:\\Users\\user\\OneDrive - Charité - Universitätsmedizin Berlin\\Documents\\coding\\80K_analysis\\qc_plots\\barcodes\\NGN2_2.merged.config.standardConfig.tsv.gz",
  replicates = "1,2",
  mindnacounts = 1,
  minrnacounts = 1,
  outdir = "barcode_test"
...
# opt <- list(
#   condition = "NGN2",
#   files = "C:\\Users\\user\\OneDrive - Charité - Universitätsmedizin Berlin\\Documents\\coding\\80K_analysis\\qc_plots\\barcodes\\NGN2_1.merged.config.standardConfig.tsv.gz,C:\\Users\\user\\OneDrive - Charité - Universitätsmedizin Berlin\\Documents\\coding\\80K_analysis\\qc_plots\\barcodes\\NGN2_2.merged.config.standardConfig.tsv.gz,C:\\Users\\user\\OneDrive - Charité - Universitätsmedizin Berlin\\Documents\\coding\\80K_analysis\\qc_plots\\barcodes\\NGN2_3.merged.config.standardConfig.tsv.gz",
#   replicates = "1,2,3",
#   mindnacounts = 1,
#   minrnacounts = 1,
#   outdir = "test"
...
if (data %>% nrow() > 1) {
  ...
  print("reps")
  for (i in seq(1, dim(selected)[2])) { # is a smart way to iterate over the pairwise combinations of replicates
    print(selected[, i])
    r1 <- selected[1, i]
    r2 <- selected[2, i]
