...
global_plot_height <- 17
global_plot_width <- 29
global_plot_legend_nrow <- 6

# for testing only two replicates without legend:
opt <- list(
  condition = "NGN2",
  label = "C:\\Users\\user\\OneDrive - Charité - Universitätsmedizin Berlin\\Documents\\coding\\80K_analysis\\qc_plots\\design_no_duplicates_sequence_and_header_label.tsv",
  files = "C:\\Users\\user\\OneDrive - Charité - Universitätsmedizin Berlin\\Documents\\coding\\80K_analysis\\qc_plots\\insert\\NGN2_3_merged_assigned_counts.tsv.gz,C:\\Users\\user\\OneDrive - Charité - Universitätsmedizin Berlin\\Documents\\coding\\80K_analysis\\qc_plots\\insert\\NGN2_1_merged_assigned_counts.tsv.gz",
  replicates = "1,2",
  threshold = 10,
  outdir = "testNoLegend"
)
...
plot_correlations_dna <- function(data, condition, r1, r2, name) {
    dna_p <-
        ...
        guides(fill=guide_legend(nrow=global_plot_legend_nrow, byrow=TRUE))
    return(dna_p)
}
plot_correlations_rna <- function(data, condition, r1, r2, name) {
    rna_p <-
        ...
        geom_point(aes(colour = label.x)) + #show.legend = TRUE
      # todo: change name of label shown in the legend
        xlim(-5, 5) +
        ylim(-5, 5) +
        xlab(sprintf(
...
write_correlation_plots <- function(plots, name) {
    ...
    ggplot2::ggsave(name,
                    ...
                    height = global_plot_height * length(plots))
}
...
if (data %>% nrow() > 1 && nrow(all) > 1) {
    ...
    plots_correlations_rna_list <- list()
    plots_correlations_dna_list <- list()
    plots_correlations_ratio_list <- list()
    ...
    write_correlation_plots(plots_correlations_dna_list,
                            sprintf("%s_DNA_pairwise.png", outdir))
    write_correlation_plots(plots_correlations_rna_list,
                            sprintf("%s_RNA_pairwise.png", outdir))
    write_correlation_plots(plots_correlations_ratio_list,
